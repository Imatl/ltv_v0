...
contract UpdateRolesTest is BaseTest {
    ...
    function test_checkOldAdministratorCantExecuteItAnymoreBatch(DefaultTestData memory data, address newAddress) public {
        vm.assume(newAddress != address(0));
        vm.assume(newAddress != data.owner);
        vm.assume(newAddress != data.guardian);
        vm.assume(newAddress != data.governor);
        ...
        testWithPredefinedDefaultValues(data)
    {
        ...
        borrowToken.approve(address(ltv), type(uint256).max);
        vm.expectRevert(abi.encodeWithSelector(IAdministrationErrors.OnlyEmergencyDeleveragerInvalidCaller.selector, oldEmergencyDeleverager));
        ltv.deleverageAndWithdraw(realBorrowAssets, 0);
        vm.stopPrank();
    }
    ...
    function checkSelfAssignmentGovernor(DefaultTestData memory data)
        internal
        testWithPredefinedDefaultValues(data)
    {
        vm.startPrank(data.owner);
    ...
    function checkSelfAssignmentGuardian(DefaultTestData memory data)
        internal
        testWithPredefinedDefaultValues(data)
    {
        vm.startPrank(data.owner);
